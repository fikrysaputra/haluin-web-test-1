<x-app-layout>
    ...
    <div class="card h-full p-4 sm:p-8 bg-white shadow sm:rounded-lg dark:bg-gray-800">
        <div class="card-body">
            <div class="relative">
                ...
                @endif
                <div class="mb-4">
                <table id="table-user-cms" class="display">
                    <thead>
                        <tr>
                            <th>
            ...
            <form id="bulkDeleteForm" method="POST" action="{{ route('user-cms.bulk-delete') }}" style="display: none;">
                @csrf
                @method('DELETE')
    ...
    <script type="text/javascript">
        ...
        var table = $('#table-user-cms').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route("api.user-cms") }}',
            columns: [
                {
                    data: null,
                    name:'id',
                    ...
                    render: function (data, type, row, meta) {
                        return '<input type="checkbox" class="user-cms-checkbox" value="' + row.id + '">';
                    }
                },
                { 
                    data: null, 
        ...
        $('#selectAll').on('change', function () {
            ...
            $('.user-cms-checkbox').prop('checked', checked);
            toggleBulkDeleteButton();
        });

        // Handle individual checkbox selection
        $('#table-user-cms tbody').on('change', '.user-cms-checkbox', function () {
            toggleBulkDeleteButton();
        });
        ...
        function toggleBulkDeleteButton() {
            var selectedCount = $('.user-cms-checkbox:checked').length;
            $('#bulkDeleteIds').val($('.user-cms-checkbox:checked').map(function () {
                return this.value;
            }).get().join(','));
        ...
        $('#bulkDeleteForm').on('submit', function (e) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: 'You want to delete the selected users?',
                icon: 'warning',
