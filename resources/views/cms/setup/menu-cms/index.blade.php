<x-app-layout>
    ...
    <div class="card h-full p-4 sm:p-8 bg-white shadow sm:rounded-lg dark:bg-gray-800">
        <div class="card-body">
            <div class="relative">
                ...
                @if (session('status') === 'menu-cms-updated')
                <div class="bg-teal-100 border text-sm text-teal-500 rounded-md p-3 mb-3" role="alert">
                    <span x-data="{ show: true }" x-show="show" x-transition class="font-bold">
                        {{ __('Menu CMS Updated Sucessfully.') }}
                    </span>
                </div>
                ...
                @endif
                <div class="mb-4">
                <table id="table-menu-cms" class="display">
                    <thead>
                        <tr>
                            <th>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#table-menu-cms').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route("api.menu-cms") }}',
                columns: [
                    {
                        data: null,
                        name:'id',
            ...
            $('#selectAll').on('change', function () {
                ...
                $('.menu-cms-checkbox').prop('checked', checked);
                toggleBulkDeleteButton();
            });

            // Handle individual checkbox selection
            $('#table-menu-cms tbody').on('change', '.menu-cms-checkbox', function () {
                toggleBulkDeleteButton();
            });
            ...
            function toggleBulkDeleteButton() {
                var selectedCount = $('.menu-cms-checkbox:checked').length;
                $('#bulkDeleteIds').val($('.menu-cms-checkbox:checked').map(function () {
                    return this.value;
                }).get().join(','));
           ...
           $('#bulkDeleteForm').on('submit', function (e) {
                ...
                if (!selectedIds) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'No Selection',
                        text: 'Please select at least one menu-cms to delete.'
                    });
                    return;
                }
                ...
                Swal.fire({
                    title: 'Are you sure?',
                    text: 'You want to delete the selected menu-cms?',
                    icon: 'warning',
